...
const Loader = ({ message }) => (
  <div className="flex h-full min-h-64 w-full flex-col items-center justify-center rounded-xl bg-white p-10">
    <div className="mb-6 flex space-x-2">
      <div
        className="h-4 w-4 animate-pulse rounded-full bg-purple-600"
...
const Perfil = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      fetch(
        `https://trajeto-cultural-backend.onrender.com/usuario/get_pontos?id=${user.user_id}`,
      )
        .then((res) => res.json())
        .then((data) => {
          setPontos(data);
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="mb-10 flex h-6 w-full max-w-xs items-center justify-start rounded-full bg-purple-200">
              <div className="relative h-6 w-full max-w-xs rounded-full bg-purple-200">
                <div
                  className="absolute left-0 top-0 h-6 rounded-full bg-purple-500 transition-all duration-500"
            ...
            <div className="mb-12 flex w-full justify-center gap-4">
              {conquistas.slice(-3).map((c) => (
                <div
                  key={c.id}
                  className="flex items-center justify-center rounded-xl bg-purple-400 px-4 py-3 text-center text-sm text-white shadow-md"
                >
                  {c.nome_conquista}
                </div>
              ))}
