...
export default function InstallPWA() {
  ...
  return (
    <div className="fixed bottom-5 left-1/2 z-[9999] flex w-[90%] -translate-x-1/2 animate-fade-in flex-col items-center gap-3 rounded-xl bg-neutral-100 px-4 py-3 text-white shadow-lg sm:w-auto sm:flex-row sm:gap-4 sm:px-5 sm:py-4">
      ...
      <button
        onClick={handleClose}
        className="absolute right-3 top-2 text-purple-600/80 transition hover:text-purple-600"
        aria-label="Fechar"
      >
        <X size={18} />
      </button>

      <p className="pr-6 text-center text-sm text-purple-500 sm:text-left sm:text-base">
        Instale nosso aplicativo em seu dispositivo!
      </p>

      <button
        onClick={handleInstallClick}
        className="mr-3 w-full rounded-lg bg-orange-500 px-4 py-2 font-semibold text-white transition hover:bg-orange-400 sm:w-auto"
      >
        Instalar
      </button>
    </div>
